...
<script type="text/javascript">
	...
	function getSmsCode() {
		...
		$.ajax({
			type : "POST",
			url : "/wireless/app/wifi/auth/sms/getcode",
			dataType : "json",
			data : {
	...
	function verifSmsCode() {
		if (!(/^\d{6}$/.test($('#smsCode').val()))) {
			...
			$.ajax({
				type : "POST",
				url : "/wireless/app/wifi/auth/sms/verif",
				dataType : "json",
				data : account,
				success : function(data) {
					if (data.success) {
			...
			<div class="row">
				<div class="col-sm-4 col-sm-offset-4" style="text-align: center;">
					<span id="wait" style="display: none;"><img
						style="width: 17px;" alt="" src="../../../../resources/image/wait.gif">&nbsp;正在认证，请等待。</span>&nbsp;<a
						style="visibility: hidden;" id="go" href="https://www.baidu.com/">注册成功，开始访问网络！</a>
				</div>
			</div>
		</div>
	</div>
